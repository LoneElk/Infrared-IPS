...
map_objects = {'type':'FeatureCollection','features':[],'properties': {'scale':SCALE,'name':'IPS Example Floor Plan',
'srcImage':'/Users/jim/Dropbox/Documents/Msc/Thesis/A4/Infrared-IPS/location_rules_engine/utilities/floor_plan/floorplan1.jpg','imageSize':list(img.shape),
'position':{'x':517910,'y':204101,'heading':-21.50,'referenceFrame':'eastingNorthing'}}}

for wall in walls:
    x,y,w,h = wall['bbox']
    bbox = [x,y,x+w,h+y]
    map_objects['features'].append({ 'type': 'Feature', 'geometry': { 'type': 'Polygon', 'coordinates': [[x[0] for x in wall['cnt']]] }, 'properties': { 'name':
    'walls','level':0,'geomType':'wall','accessible':False,'colour':(0,0,0)},'bbox':bbox })
...
for key,room in rooms.items():
 ...
 'level':0,'geomType':'floor','accessible':True,'colour':room['colour']},'bbox':bbox }  
    )
